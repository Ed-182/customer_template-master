with payments as (
    select * from {{ ref('stg_payments') }}
),

pivoted as (
    select 
        order_id,
        round(case when payment_method = 'bank_transfer' then amount else 0 end) as bank_transfer_amount,
        round(case when payment_method = 'coupon' then amount else 0 end) as coupon_amount,
        round(case when payment_method = 'credit_card' then amount else 0 end) as credit_card_amount,
        round(case when payment_method = 'gift_card' then amount else 0 end) as gift_card_amount
    from payments
    where status = 'success'
    group by 1
)

select * from pivoted
 ORDER BY order_id;